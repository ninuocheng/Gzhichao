...
#一开始主要是针对存储的car文件没有拷贝好，现在相对比较完善了，不管是存储的car有没有拷贝好，都可以执行脚本自动导入
...
[ ! -d "$ScriptDir" ] && echo "${ScriptDir}不存在，请检查。" && exit
#注意导入订单的文件，注意把要导入的订单文件拷贝命名为import-deal-file，建议不要改动脚本
ImportDealFile=import-deal-file
[ ! -f "$ScriptDir/$ImportDealFile" ] && echo "$ScriptDir/${ImportDealFile}不存在，请检查。" && exit
...
[ "$InitialNub" -eq 0 ] &&  echo "准备的文件$ScriptDir/$ImportDealFile要导入的订单数量为0，请检查。" && exit
#导入订单的日志
ImportDealLog="$ScriptDir/import-deal.log"
#备份日志的时间
#BakLogTime=`date +%Y%m%d-%H%M%S`
#[ -f "$ScriptDir/import-deal.log" ] && mkdir -p $ScriptDir/logbak && mv $ScriptDir/import-deal.log $ScriptDir/logbak/import-deal-${BakLogTime}.log
#boost导入订单的日志文件
#BoostLog="/opt/raid0/boost/logs"
#[ ! -f "$BoostLog" ] && echo "boost导入订单的日志文件${BoostLog}不存在，请检查。" && exit
echo "本轮初始导入订单的开始时间：$(date +%F%\t%T)" |tee -a $ImportDealLog
...
do
     ...
     do
        ...
        ImportFrontNub=`wc -l $ScriptDir/$ImportDealFile |awk '{print $1}'`
     	...
     	#awk -F'"' '{print $2,$4}' $ScriptDir/$ImportDealFile | while read UUid PieceCid
     	#awk '{print $1,$2}' $ScriptDir/$ImportDealFile | while read UUid PieceCid
		...
		#查看PieceCid是否唯一
			...
			#如果导入订单有报错，会先睡眠一段时间，等待检查报错原因,一般就是余额不足导致的，充币或者是等导入的订单上链，余额释放就可以了，可以不中断脚本，脚本会继续当前报错的订单开始重新导入
				       ...
				       sleep 600s
                        ...
                        #建议测试几个，会睡眠一段时间，检查导入的订单是否封装正常
			[ "${DealNub}" -eq 1 ] && sleep 6s
                        #[ "${DealNub}" -eq 20 ] && sleep 6s
                else
                        continue
                fi
     		#更新保存已导入订单的数量
		...
		#这里的停顿主要是为缓解服务器的压力，更是为了避免boost吞订单的情况,停顿的时间看着合适就好
     		#sleep 0.5s
     	        #刚开始执行脚本导入订单方便观察，如果异常，停顿的这15秒足够中断脚本了
     	        [ "${DealNub}" -eq 1 ] && sleep 15s
	done
     	#已循环次数的计数
     	let CycleNub++
     	#输出已循环的次数
        ...
        [ "$DealNub" -gt 0 ] && echo "已导入的订单数量：$DealNub" |tee -a $ImportDealLog
     	...
     	#[ "$NewWillImportNub" -gt 0 ] && echo "新增要导入的订单数量：$NewWillImportNub"
     	[ "$NewWillImportNub" -gt 0 ] && echo "新增要导入的订单数量：$NewWillImportNub" && continue
     	#检查还剩余要导入的订单数量如果为0，就直接退出当前的循环
        [ "$RemainderNub" -eq 0 ] &&  break
     	echo "还剩余要导入订单存储的car文件正在拷贝中，请耐心等待...................................................................................................." |tee -a $ImportDealLog
     	#睡眠一段时间，再循环导入剩余的订单
     	sleep  1800s
     done
     #检查还剩余要导入的订单数量；如果为0，就会睡眠一段时间，再继续进行下一次的循环。直到循环了3次就退出脚本；目的主要是为了测试的不中断执行脚本
     #let gocycle++
     #[ "$gocycle" -le 3 ] && {
#	sleep 100s
 #       continue
